...
class DeepCopyWithPositions {
  ...
  def apply(x: Stmt): Stmt = {
    pos(x match {
      ...
      case Invoke(ref, method, args, res, invs) => Invoke(this (ref), method, this (args), res.map(this (_)), invs.map(this (_)))
      ...
      case While(cond, stmt, invs, decreases)     => While(this (cond), this (stmt), this (invs), this (decreases))
      ...
      case Yield(ensures)                         => Yield(this (ensures))
      case Commit()                               => Commit()
...
object DeepCopyWithPositions extends DeepCopyWithPositions