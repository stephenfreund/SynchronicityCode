...
public class TCuckooHashSet {
  ...
  public TCuckooHashSet(int capacity) {
    ...
    table = (int[][]) new Object[2][capacity];
    size = capacity;
    for (int i = 0; i < 2; i++) {
  ...
  private int swap(int i, int j, int value) {
    int result = table[i][j];
    table[i][j] = value;
    return result;
  }
  
  public boolean optContains(TreeSet<Intent>[] intentions, int x) {
    int h0 = hash0(x);
    int y = table[0][h0];
  ...
  boolean validate(TreeSet<Intent>[] intentions, List<Lock> acquired) {
    for (int phase = 0; phase < 2; phase++) {
      for (Intent intent : intentions[phase]) {
        ...
        int expected = intent.item;
        int found = table[phase][intent.index];
        if (expected == null) {
          if (found != null) {
            return false;
          }
  ...
  boolean optAdd(TreeSet<Intent>[] intentions, int x) {
