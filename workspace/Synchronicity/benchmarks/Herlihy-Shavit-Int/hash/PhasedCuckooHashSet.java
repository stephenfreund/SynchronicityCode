 ...
 */
public abstract class PhasedCuckooHashSet {
  /**
  ...
  public PhasedCuckooHashSet(int size) {
    capacity = size;
    table = (List[][]) new java.util.ArrayList[2][capacity];
    for (int i = 0; i < 2; i++) {
   ...
   * @return non-negative hash value
   */
  final public synchronized int hash1(int x) {
  ...
  public boolean remove(int x) {
    acquire(x);
    try {
      List set0 = table[0][hash0(x) % capacity];
      if (set0.contains(x)) {
        set0.remove(x);
        return true;
      } else {
        List set1 = table[1][hash1(x) % capacity];
        if (set1.contains(x)) {
          set1.remove(x);
          return true;
        }
  ...
  public boolean add(int x) {
    ...
    try {
      ...
      List set0 = table[0][h0];
      List set1 = table[1][h1];
